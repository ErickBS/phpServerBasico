			...
			<div  class="column">
				...
				<nav class="navbar navbar-light bg-light">
				  <div class="container-fluid">
					<form class="d-flex" action="pesquisa_empresa.php" method="POST">
						<input class="form-control me-2" type="search" placeholder="Empresa" aria-label="Search" name="busca" autofocus>
				...
				<table class="table table-hover" id="myTable">
					 ...
					 <thead>
						...
						<!-- Aqui, a tabela está executando a função sortTable, que está no fim do documento, e também inclui o ícone de sort para indicar que existe a função.-->
						  <th scope="col" onclick="sortTable(0)">Empresa <i style="text-align:right" class="fa fa-fw fa-sort"></i></th>
						  <th scope="col" onclick="sortTable(1)">UF <i style="text-align:right" class="fa fa-fw fa-sort"></i></th>
						  <th scope="col" onclick="sortTable(2)">CNPJ <i style="text-align:right" class="fa fa-fw fa-sort"></i></th>
						...
						<?php
							while ($linha = mysqli_fetch_assoc($dados) ) {
								...
								echo "<tr>
									  ...
									  <td>$CNPJ</td>
									  <td width=150px>
									  <!-- Para editar, estou usando um simples GET via URL -->
										<a href='cadastro_empresa_edit.php?id=$CNPJ' class='btn btn-success btn-sm'>Editar</a>
										<!-- E para deletar, um modal embutido na página para dar a chance de se arrepender. -->
										<a href='#' class='btn btn-danger btn-sm' data-bs-toggle='modal' data-bs-target='#confirma' onClick=".'"'."pegar_dados($CNPJ, '$razaoSocial')".'"'.">Excluir</a>
									</tr>";
							}
						?>							
	  ...
	  <div class="modal-dialog">
		<div class="modal-content">
		  ...
		  <div class="modal-body">
			<form action="empresa_excluir_script.php" method="POST">
			<p>Deseja realmente excluir <b id="razaoSocial">Nome da pessoa</b>? </p>
			
		  </div>
		  <div class="modal-footer">
			<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Não</button>
			<input type ="hidden" name="id" id="CNPJ" value="">
			<input type ="hidden" name="nome" id="razao" value="">
...
<!-- Uma função apenas para acertar os dados enviados de uma página para outra..-->
	<script type="text/javascript">
		function pegar_dados(id, nome){
			document.getElementById('razaoSocial').innerHTML = nome;
			document.getElementById('razao').value = nome;
